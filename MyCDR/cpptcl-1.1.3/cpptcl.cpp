...
void details::set_result(Tcl_Interp *interp, std::string const &s)
{
...
typedef std::map<std::string, boost::shared_ptr<callback_base> > callback_interp_map;
...
typedef std::map<std::string, boost::shared_ptr<class_handler_base> > class_interp_map;
...
policies Tcl::factory(std::string const &name)
{
...
result interpreter::eval(std::istream &s)
{
     std::string str(
