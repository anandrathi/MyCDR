...
public:
	...
	_Myt& operator=(const _Valty& _Val)
		...
		{
			ACE_DEBUG ((LM_DEBUG, "(%D)(%t) back_insert_iterator_ignoredir::Collect operator= %s is a file  \n",  _Val.string().c_str() ));	
			container->push_back(_Val);
		}else
		{
			ACE_ERROR ((LM_ERROR, "(%D)(%t) back_insert_iterator_ignoredir::Collect operator= %s is a file  \n",  _Val.string().c_str() ));	
		}
		return (*this);
		}
...
bool DirMoniter::testPath(std::string& tpath)
{
	...
	{
		ACE_DEBUG ((LM_DEBUG, "(%D)(%t) DirMoniter::testPath path %s found\n",  tpath.c_str()  ));	
		return true;
	}
	else{
		ACE_DEBUG ((LM_DEBUG, "(%D)(%t) DirMoniter::testPath path %s NOT found\n",  tpath.c_str()  ));	
	}
  ...
  try
  {
	  ...
	  if (boost::filesystem::exists(path))    // does p actually exist?
	  {
		...
		else if (boost::filesystem::is_directory(path))      // is p a directory?
		{
			...
			{
				ACE_DEBUG ((LM_DEBUG, "(%D)(%t) DirMoniter::Collect		%s\n",  it->string().c_str() ));	
				//std::cout << "   " << *it << '\n';
			}
		}
		else
		{
			ACE_ERROR ((LM_ERROR, "(%D)(%t) Error DirMoniter::Collect %s	exists, but is neither a regular file nor a directory	\n",  path.c_str() ));	
			return -1;
		}
	...
	{
		ACE_ERROR ((LM_ERROR, "(%D)(%t) Error DirMoniter::Collect %s does not exist\n",  path.c_str() ));	
		return -1;
	}
  }
  catch (const boost::filesystem::filesystem_error& ex)
  {
	ACE_ERROR ((LM_ERROR, "(%D)(%t) Error DirMoniter::Collect %s \n",  ex.what() ));	
    std::cout << ex.what() << '\n';
	return -1;
  }
...
{
	ACE_TRACE("(%D)(%t) DirMoniter::DirMoniter");
	path=p;
}
...
void DirMoniter::rename(std::string from, std::string to)
{
	...
	try
	{
		...
		ACE_DEBUG ((LM_DEBUG, "(%D)(%t) DirMoniter::rename %s %s \n",  from.c_str(), to.c_str() ));
	}
	catch(const boost::filesystem::filesystem_error& ex)
	{
		ACE_ERROR ((LM_ERROR, "(%D)(%t) Error DirMoniter::rename (%s) %s %s \n",  ex.what(), from.c_str(), to.c_str() ));	
		//std::cout << ex.what() << '\n';
	}
}
...
{
	ACE_DEBUG ((LM_DEBUG, "(%D)(%t)DirMoniter::dump Path = %s\n" , this->path.c_str() ));	
    for (PATHVEC::const_iterator it (_pathvec.begin()); it != _pathvec.end(); ++it)
    {
		ACE_DEBUG ((LM_DEBUG, "(%D)(%t) DirMoniter::dump  file = %s \n",  it->string().c_str() ));	
    }
}
